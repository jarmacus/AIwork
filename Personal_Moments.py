...
from pyui.settingArea import Ui_Form as _SettingHome
...
from utils.database_utils import get_province_from_table, get_city_from_table, \
    ...
    query_subclass, rename_subclass, database_test, top5_search, image_group_by_top5
...
#     try:
#         result = cursor.execute("SELECT * FROM PersonTable LIMIT 1").fetchall()
#         print("PersonTable is ready with data:", result)
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def closeEvent(self, event):
        self.date_process_thread.quit()
        self.date_process_thread.wait()
        # 同样处理其他线程
        CONN.close()
...
class ThingForm(QStackedWidget):
    def __init__(self):
        ...
        # 信号与槽连接
        self.thing_home.pushButton_search.clicked.connect(self.show_image)  # 搜索按钮
        self.thing_image.pushButton_return.clicked.connect(self.show_home)  # 返回按钮
        self.search_image.pushButton_return.clicked.connect(self.show_home)  # 返回按钮
        self.thing_home.image_area.tableWidget_image.cellDoubleClicked.connect(self.show_subclass)  # 双击显示子类
    ...
    def show_image(self):
        ...
        Show images in searching result.
        """
        success_flag, self.search_image.image_list = self.thing_home.search()
        if not success_flag:
            _ = QMessageBox.warning(self, "Warning", "You should enter a name in the first line", QMessageBox.Ok,
                                    QMessageBox.Ok)
            return
        self.setCurrentIndex(2)
        self.search_image.refresh()
...
class ThingHome(QWidget, _ThingHome):
    ...
    def search(self):
        ...
        self.logic = self.comboBox_logic.currentText()
        self.p2 = self.lineEdit_p2.text()
        return True, thing_search(CURSOR, self.p1, self.logic, self.p2)
#end
...
class SettingForm(QStackedWidget):
    def __init__(self):
        super(SettingForm, self).__init__()

        # 初始化子页面，添加到堆叠控件
        self.setting_home = SettingHome(table_name="Top5", like=True, rename=False)
        self.setting_image = SearchImage(table_name="Top5", like=True, delete=True)
        self.search_image = SearchImage(table_name="Top5", like=True, delete=False)
        self.addWidget(self.setting_home)
        self.addWidget(self.setting_image)
        self.addWidget(self.search_image)

        # 信号与槽连接
        self.setting_home.pushButton_search.clicked.connect(self.show_image)  # 搜索按钮
        self.setting_image.pushButton_return.clicked.connect(self.show_home)  # 返回按钮
        self.search_image.pushButton_return.clicked.connect(self.show_home)  # 返回按钮
        self.setting_home.image_area.tableWidget_image.cellDoubleClicked.connect(self.show_subclass)  # 双击显示子类
    ...
    def show_subclass(self):
        ...
        Show subclass page.
        """
        self.setting_image.image_list, self.setting_image.subclass_name = self.setting_home.image_area.query_subclass()
        if self.setting_image.image_list is None:
            return
        self.setCurrentIndex(1)
        self.setting_image.refresh()
    ...
    def show_image(self):
        ...
        Show images in searching result.
        """
        success_flag, self.search_image.image_list = self.setting_home.search()
        if not success_flag:
            _ = QMessageBox.warning(self, "Warning", "You should enter a name in the first line", QMessageBox.Ok,
                                    QMessageBox.Ok)
            return
...
class SettingHome(QWidget, _SettingHome):
    def __init__(self, table_name="Top5", like=True, rename=False):
        """
        物品搜索页面。
        Thing searching page.
        """
        super(SettingHome, self).__init__()
        self.setupUi(self)
    ...
    def search(self):
        ...
        self.logic = self.comboBox_logic.currentText()
        self.p2 = self.lineEdit_p2.text()
        return True, top5_search(CURSOR, self.p1, self.logic, self.p2)
...
class LabeledImage(QWidget):
    def __init__(self, info_list, item_size=200):
        ...
        if len(info_list) > 1:
            ...
            self.text = "Default Text"  # 如果没有足够的元素，设置默认值
        print(info_list)  # 调试，查看 info_list 的内容
        if len(info_list) > 2:
            self.image_file = info_list[2]
        else:
            self.image_file = "default_image.jpg"  # 设置一个默认值或做其他处理
