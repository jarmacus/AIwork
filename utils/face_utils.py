...
FEATURE_DIM = 128
TEXT_DIR = "C:\\Users\\86178\\Desktop\\Personal_Moments-master\\record"
...
def get_face_feature(img, face, method):
    ...
    # face_feature = face_feature.reshape(1, -1)  # 将特征向量转为一行128列的形状
    return face_feature
...
def load_feature_centers(csv_file=FEATURE_CSV_FILE):
    ...
    feature_centers = feature_centers.reshape(-1,128)
...
def load_face_numbers(txt_file=FACE_NUMBER_TXT_FILE):
    ...
    :param txt_file: txt 文件路径，默认使用 FACE_NUMBER_TXT_FILE
    :return: 返回包含人脸数量的整数列表
    """
    # 检查文件是否存在
    ...
    try:
        ...
        # 去除每行末尾的换行符，并忽略空行
        face_numbers = [int(number.rstrip()) for number in face_numbers if number.strip()]
...
def load_face_names(txt_file=FACE_NAME_TXT_FILE):
    ...
    :param txt_file: txt 文件路径，默认使用 FACE_NAME_TXT_FILE
    :return: 返回包含人脸名称的字符串列表
    """
    # 检查文件是否存在
    if not os.path.exists(txt_file):
        print(f"File {txt_file} does not exist.")
    ...
    try:
        ...
        # 去除每行末尾的换行符，并忽略空行
        face_names = [name.rstrip() for name in face_names if name.strip()]
...
def save_face_names(face_names, txt_file=FACE_NAME_TXT_FILE):
    ...
    :return None:
    """
    print("yuan:",face_names)
    face_names = [name + "\n" for name in face_names]
    # print("hou:", face_names)
    with open(txt_file, "w") as f:
