...
from utils.setting_utils import load_top5_names
from utils.thing_utils import load_thing_names
from .face_utils import load_face_names

# 用于表格创建的语句
# SQL strings for table creation
TABLE_CREATION_DICT = {
    "Date": """CREATE TABLE IF NOT EXISTS Date (image_file TEXT PRIMARY KEY)""",
    "Place": """CREATE TABLE IF NOT EXISTS Place (image_file TEXT PRIMARY KEY)""",
    "Thing": """CREATE TABLE IF NOT EXISTS Thing (image_file TEXT PRIMARY KEY)""",
    "People": """CREATE TABLE IF NOT EXISTS People (image_file TEXT PRIMARY KEY)""",
    "Top5": """CREATE TABLE IF NOT EXISTS Top5 (image_file TEXT PRIMARY KEY)""",
    "Summary": """CREATE TABLE IF NOT EXISTS Summary (
        image_file TEXT PRIMARY KEY, 
        md5 CHAR(32), 
        create_date DATE, 
        image_name_date TEXT, 
        create_week CHAR(22), 
        image_name_week TEXT, 
...
# 表格集合
TABLES = set(["Date", "Place", "Thing", "People", "Summary", "Top5"])
 # for table name checking
...
def all_search(cursor, start_date, end_date, province, city, district, people, thing):
    ...
    if not _field_is_none(thing):
        conditions.append("image_name_thing LIKE %s" % _add_quotation(_add_percent_sign(thing)))
...
def thing_search(cursor, p1, logic, p2):
    """
    ...
    :return results: image file list, list
    """
    sql_string = "SELECT image_file FROM Summary WHERE image_name_thing LIKE ?"
    if _field_is_none(p2):  # only one condition
        sql_string += " ORDER BY create_date"
        results = cursor.execute(sql_string, tuple([_add_percent_sign(p1)])).fetchall()
    else:  # two conditions need to be used
        if logic == "AND":  # p1 and p2
            sql_string += " AND image_name_thing LIKE ? ORDER BY create_date"
        elif logic == "OR":  # p1 or p2
            sql_string += " OR image_name_thing LIKE ? ORDER BY create_date"
        elif logic == "NOT":  # p1 not p2
            sql_string += " AND image_name_thing NOT LIKE ? ORDER BY create_date"
        else:
            raise ValueError
        results = cursor.execute(sql_string, tuple([_add_percent_sign(p1), _add_percent_sign(p2)])).fetchall()
    return _process_empty_result(results, "Thing Search")

def top5_search(cursor, p1, logic, p2):
    """
    根据表Summary中的top5字段是否包含key搜索图片文件，返回结果按照时间排序。
    Search from image file by judging whether keyword is in top5 field in table Summary.
    Returned results are ordered by create date.
    ...
    if _field_is_none(p2):  # only one condition
        ...
        if logic == "AND":  # p1 and p2
            sql_string += " AND top5 LIKE ? ORDER BY create_date"
        elif logic == "OR":  # p1 or p2
            sql_string += " OR top5 LIKE ? ORDER BY create_date"
        elif logic == "NOT":  # p1 not p2
            sql_string += " AND top5 NOT LIKE ? ORDER BY create_date"
        else:
            raise ValueError
        results = cursor.execute(sql_string, tuple([_add_percent_sign(p1), _add_percent_sign(p2)])).fetchall()
    return _process_empty_result(results, "Top5 Search")
...
def table_exist(cursor, table, output=False):
    ...
    if len(result) == 0:
        # create target table
        table_creation = TABLE_CREATION_DICT[table]
...
def image_group_by_person(cursor):
    ...
    for person in person_list:
        query_results = cursor.execute(sql_string, tuple([_add_percent_sign(person)]))
        fetched_results = query_results.fetchall()
        if fetched_results:  # 检查是否有结果
            result = fetched_results[0]  # 访问第一个结果
            results.append(("person", person, result[0]))
        else:
            print(f"No records found for person: {person}")
    return results
...
def image_group_by_thing(cursor):
    ...
    """
    results = []
    thing_list = load_thing_names(cursor)
    thing_list.sort()
    sql_string = "SELECT image_file FROM Summary WHERE image_name_thing like ?"
    for thing in thing_list:
        result = cursor.execute(sql_string, tuple([_add_percent_sign(thing)])).fetchall()[0]
        results.append(("image_name_thing", thing, result[0]))
    return results


def image_group_by_top5(cursor):
    """
    通过thing标签对图片进行分组，为每一个条目返回("top5", thing标签，图片文件)。
    Group image file by thing name, return "top5", thing name and image file for each item.
    ...
    """
    results = []
    top5_list = load_top5_names(cursor)
    # 过滤掉空字符串项
    top5_list = [top5[0] for top5 in top5_list if top5[0]]  # 只保留非空的top5值
    top5_list.sort()
    print(top5_list)
    sql_string = "SELECT image_file FROM Summary WHERE top5 like ?"

    for top5 in top5_list:
        top5_like = _add_percent_sign(top5)
        result = cursor.execute(sql_string, (top5_like,)).fetchall()

        if result:
            results.append(("top5", top5, result[0][0]))  # result[0][0] is the image file
...
def database_test(output=True):
    ...
    cursor = conn.cursor()
    for table in ["Date", "Place", "Thing", "People", "Summary","Top5"]:
